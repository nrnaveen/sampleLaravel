...
@section('content')
	{{ Form::model($client, array('url' => array('/admin/clients/edit/' . $client->id), 'novalidate' => 'novalidate', 'id' => 'updateclient', 'class' => 'form-horizontal',)) }}
		@if (count($errors) > 0)
		...
		<div class="box">
			<div class="box-body">
				<div class="form-group">
					...
					<div class="col-sm-10">
						{{ Form::text('firstname', null, array('class' => 'form-control', 'id' => 'firstname', 'placeholder' => trans('messages.FirstName'))) }}
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-10">
						...
						<select id="color" name="color" class="form-control" required="">
							...
							@foreach($colors as $key => $color)
								<option style="background-color: {{$key}};" value="{{$key}}" {{ $key === $client->color? 'selected' : '' }}>{{$color}}</option>
							@endforeach
						</select>
					</div>
				</div>
...
@section('content_header')
	<h1> {{trans('messages.EditClient')}}<small>{{trans('messages.ModifyClientDetails')}}</small></h1>
	<ol class="breadcrumb">
